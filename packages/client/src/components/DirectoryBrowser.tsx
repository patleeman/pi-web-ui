...
export function DirectoryBrowser({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 md:p-4">
      <div className="bg-pi-bg border border-pi-border max-w-2xl w-full max-h-[90vh] md:max-h-[80vh] flex flex-col font-mono text-sm">
        {/* Header */}
        ...
        <div className="border-b border-pi-border px-3 md:px-4 py-2 text-pi-muted overflow-x-auto">
          <span className="text-xs whitespace-nowrap">{displayPath}</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {parentPath && (
            <button
              ...
              className="w-full px-3 md:px-4 py-3 md:py-2 text-left hover:bg-pi-surface active:bg-pi-surface flex items-center gap-2 border-b border-pi-border/50"
            >
          ...
          {currentPath === '/' && (
            <div className="px-3 md:px-4 py-2 text-xs text-pi-muted border-b border-pi-border/50">
          ...
          {entries.length === 0 ? (
            ...
            entries.map((entry) => (
              <div
                key={entry.path}
                className="flex items-center border-b border-pi-border/30 hover:bg-pi-surface active:bg-pi-surface group"
              >
                <button
                ...
                >
                  ...
                  {entry.hasPiSessions && (
                    <span className="text-pi-accent text-xs flex-shrink-0">●</span>
                  )}
                </button>
                <button
                  onClick={() => {
